...
2. Determine the number of variables (columns) in the dataset. Print this in a sentence.
...
* First, we store the predicted y values to the variable ```predicted_y```. We generate these values using the ```.predict()``` function on our model and passing the x variable of the training set as argument.
...
2. Does de model predict equally well for each transport mode?
...
3. Based on the report, could you use this model to determine if someone uses the car as their main transport mode?
4. Based on the report, could you use the model to determine if one uses the bike as their main transport mode?
